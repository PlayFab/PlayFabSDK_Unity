...
{
    Write-Host -ForegroundColor Red "Environment variable GameDKCoreLatest is not found. Please make sure Microsoft GDK is installed."
    exit 1
}

$env:dst_dir = "$PSScriptRoot\..\..\Source\DLLs\GameCore"
$env:src_party_dir = "$env:GameDKCoreLatest\ExtensionLibraries\PlayFab.Party.Cpp\Redist\CommonConfiguration\neutral"
$env:src_partyxbl_dir = "$env:GameDKCoreLatest\ExtensionLibraries\PlayFab.PartyXboxLive.Cpp\Redist\CommonConfiguration\neutral"
...
xcopy "$env:src_party_dir\Party.dll" "$env:dst_dir\Party.dll*" /Y /R /F
xcopy "$env:src_party_dir\Party.pdb" "$env:dst_dir\Party.pdb*" /Y /R /F
