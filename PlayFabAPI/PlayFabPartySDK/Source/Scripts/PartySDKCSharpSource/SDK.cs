...
namespace PartyCSharpSDK
{
    public partial class SDK
    {
            ...
            out PARTY_HANDLE handle)
        {
            using (DisposableCollection dc = new DisposableCollection())
            {
                unsafe
                {
                    ...
                    UTF8StringPtr titleIdUTF8Str = new UTF8StringPtr(titleId, dc);
                    PARTY_INITIALIZATION_CONFIGURATION initConfig = new PARTY_INITIALIZATION_CONFIGURATION(
                        titleIdUTF8Str,
                        XTaskQueueHandle.Null,
                        XTaskQueueHandle.Null);

                    UInt32 err = PFPInterop.PartyInitialize(&initConfig, out interopHandle);
            ...
            out PARTY_LOCAL_USER_HANDLE localUser)
        {
            ...
            unsafe
            {
                err = PFPInterop.PartyCreateLocalUser(
                    handle.InteropHandle,
                    entity.InteropHandle,
