...
namespace PartyCSharpSDK
{
    public static class Converters
    {
        ...
        private static Byte[] StringToNullTerminatedUTF8ByteArrayInternal(string str, int requiredByteArrayLength)
        {
            ...
            {
                int byteCount = System.Text.Encoding.UTF8.GetByteCount(str + '\0');
                if (byteCount > requiredByteArrayLength)
                {
                    throw new ArgumentException("The provided buffer length is insufficient for the UTF-8 encoded string.");
                }
